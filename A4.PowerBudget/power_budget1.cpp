...
{
  public:
    CHeater( const std::string& aName ); //cTor

    void SetHeatSetting( int aHeatSetting );  // set the heater's heat setting
    int PowerDraw();                          // power drawn, in watts
...
int main()
{
  ...
  CHeater centralheater( "CentralHeater" );
  centralheater.TurnOn();
  centralheater.SetHeatSetting( 20 );
  ...
  int heaterDraw = centralheater.PowerDraw();
  std::cout << centralheater.GetName() << ": " << heaterDraw << " W" << std::endl;
  total += heaterDraw;
...
void CHeater::SetHeatSetting( int aHeatSetting )
{
  mHeatSetting = aHeatSetting;
}
int CHeater::PowerDraw()
{
  return IsOn() ? mHeatSetting * 5 : 0;
}