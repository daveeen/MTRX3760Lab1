...
const double LowSpeed = 0.25;         // low speed before steering is added
...
struct SBattery
{
  double charge;             // battery charge, 0 to 100%
};
...
void DrainBattery( SBattery* apBattery, double amount );
...
int main()
{
  ...
  {
    DrainBattery( &battery, 10.0 ); // drain 10% of the battery each cycle
                  ...
                  SMotor* apLeftMotor, SMotor* apRightMotor, SBattery* apBattery)
{
  ...
  {
    SetMotorSpeed( apLeftMotor, LowSpeed + steering );
    SetMotorSpeed( apRightMotor, LowSpeed - steering );
  }else
  {
...
void InitBattery( SBattery* apBattery )
{
  apBattery->charge = 100; // initialize battery charge to 100%
}

// ---DrainBattery---------------------------------------------------------------
void DrainBattery( SBattery* apBattery, double amount )
{
   apBattery->charge -= amount;
   if (apBattery->charge < 0) apBattery->charge = 0; //clamp
   else if (apBattery->charge > 100) apBattery->charge = 100; //clamp
}
