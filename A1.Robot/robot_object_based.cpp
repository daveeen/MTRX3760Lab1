...
//---CBattery--------------------------------------------------------------------
// A CBattery is a single battery with a charge level.
class CBattery
{
  public:
    CBattery(); // cTor, initializes battery charge to 100%
    double GetCharge() const;
    void Drain( double amount );
  private:
    double mCharge; // battery charge, 0 to 100%
...
void CRobot::Update()
{
  ...
  double steering = mController.ComputeSteering( error );
   ...
   // drain 10% of the battery each cycle
  if( mBattery.GetCharge() <= 0.0 )
  {
    return;
  }else if( mBattery.GetCharge() < 80.0 )
  {
    mLeftMotor.SetSpeed( LowSpeed + steering );
    mRightMotor.SetSpeed( LowSpeed - steering );
  }else
  {
...
void CBattery::Drain( double amount )
 {
      mCharge -= amount;
      if (mCharge < 0) mCharge = 0; //clamp
      else if (mCharge > 100) mCharge = 100; //clamp
      }
